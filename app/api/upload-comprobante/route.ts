...
async function agregarEnSheet(
  token:         string,
  vendedorEmail: string, // ← NUEVO: Agregamos el parámetro
  titulo:        string,
  precio:        string,
  linkDrive:     string,
  fecha:         string,
  ...
  // MODIFICADO: Ahora el mail va en la Columna A (vendedorEmail)
  // Desplazamos los otros datos una columna a la derecha
  const values = [[vendedorEmail, fecha, titulo, precio, 'COMPROBANTE_ENVIADO', linkDrive]]
...
export async function POST(req: NextRequest) {
  try {
    ...
    const vendedorEmail = form.get('vendedorEmail') as string | null // ← NUEVO: Capturamos el mail del form

    if (!archivo || !titulo || !precio) {
      return NextResponse.json({ error: 'Faltan datos' }, { status: 400 })
    }

    // Si por alguna razón no viene el mail, ponemos uno por defecto para no romper el Excel
    const emailAUsar = vendedorEmail || 'mguiyemo@gmail.com'
    ...
    // MODIFICADO: Pasamos el emailAUsar a la función
    await agregarEnSheet(token, emailAUsar, titulo, precio, linkDrive, fecha)

    console.log('✅ Comprobante registrado en Pedidos para:', emailAUsar)
